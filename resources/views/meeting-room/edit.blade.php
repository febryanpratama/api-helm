...
@section('content')
{{-- Hidden Element --}}
<input type="hidden" id="meeting-room-id" value="{{ $meetingroom->id }}">
<input type="hidden" id="redirect-back" value="{{ env('SITE_URL') }}/course/show/{{ $meetingroom->course_id }}">
<input type="hidden" id="d-year" value="{{ date('Y', $meetingroom->time) }}">
<input type="hidden" id="d-month" value="{{ date('m', $meetingroom->time) }}">
<input type="hidden" id="d-day" value="{{ date('d', $meetingroom->time) }}">
	...
	<div class="clearfix bg-white card-custom mb-4">
		<div class="border-left-element">
			<div class="p-2">
				...
				<b>@lang('label.edit_meeting_room')</b>
			</div>
		</div>
	</div>
				...
				<div class="row">
					<div class="col-sm-12 col-12">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="name" id="name" placeholder="@lang('label.enter_meeting_room_name')" value="{{ $meetingroom->name }}">
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-sm-12 col-12">
						<div class="form-group">
							...
							<textarea name="description" class="form-control tinymce-element" placeholder="@lang('label.enter_meeting_room_description')" id="note-detail-task" rows="3">{{ $meetingroom->description }}</textarea>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 col-12">
						...
						<select name="is_online" id="is-online" class="form-control">
							<option value="1" @if($meetingroom->is_online == 1) selected="" @endif>Online</option>
							<option value="0" @if($meetingroom->is_online == 0) selected="" @endif>Offline</option>
						</select>
					</div>
					...
					<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 col-12">
						<div class="row">
							<div class="col-sm-12 col-md-8 col-lg-8 col-xl-8 col-12">
								...
								<input type="text" name="date" class="form-control date" id="date" placeholder="Ex: 01/01/2022" value="{{ date('m/d/Y', $meetingroom->time) }}">
							</div>

							<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4 col-12">
								<label for="">@lang('label.time') <span class="text-danger">*</span></label><br>
								<input type="text" name="time" class="form-control time" id="time" placeholder="Ex: 12:00" value="{{ date('H:i', $meetingroom->time) }}">
							</div>
						</div>
					</div>
				</div>

				<div class="form-group mt-3 online-element" @if($meetingroom->is_online == 0) style="display: none;" @endif>
					<label for="">Link <span class="text-danger">*</span></label>
					<input type="text" name="link" class="form-control" value="{{ ($meetingroom->link) ? $meetingroom->link : env('SITE_URL').'/meet/'.strtolower(str_replace(' ', '-', $meetingroom->majors->Name)).'-'.strtolower(str_replace(' ', '-', $randomString)) }}" readonly="">
				</div>

				<div class="row online-element" @if($meetingroom->is_online == 0) style="display: none;" @endif>
					<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 col-12">
						<div class="form-group">
							<label for="">@lang('label.expiration_date_link')</label>
							<input type="text" name="date_exp" class="form-control date" id="date-exp" placeholder="Ex: 01/01/2022" value="{{ date('m/d/Y', $meetingroom->link_expiration_time) }}">
						</div>
					</div>
					...
					<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 col-12">
						<div class="form-group">
							...
							<input type="text" name="time_exp" class="form-control time" id="time-exp" placeholder="Ex: 12:00" value="{{ date('H:i', $meetingroom->link_expiration_time) }}">
						</div>
					</div>
				</div>

				<div class="form-group mt-3 offline-element" @if($meetingroom->is_online == 1) style="display: none;" @endif id="input-address">
					<label for="">@lang('label.address') <span class="text-danger">*</span></label>
					<input type="text" name="address" id="address" class="form-control" placeholder="ex: Jl. Asia Afrika Bandung" value="{{ $meetingroom->address }}">
				</div>
				
				<div class="form-group mt-4">
					<button class="btn btn-sm btn-primary" id="meeting-room-btn-loading">@lang('label.edit')</button>
				</div>
			</div>
		</form>
	</div>
...
<script>
	{{-- Global Var --}}
	let dYear 	= $('d-year').val()
	let dMonth 	= $('d-month').val()
	let dDay 	= $('d-day').val()
	...
	$('.date').datepicker({
	    autoclose: true,
	    todayHighlight: true,
	    defaultViewDate: { year: dYear, month: dMonth, day: dDay }
	})
...
<script>
	...
	$(document).on('submit', '#meeting-room-area form', function (e) {
	    ...
	    $.ajax({
	        url: `${baseUrl}/meeting-room/update/${$('#meeting-room-id').val()}`,
	        type: 'POST',

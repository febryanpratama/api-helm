...
@section('content')
{{-- Hidden Element --}}
<input type="hidden" id="check-store-url" value="{{ route('management.checkout.store') }}?course-id={{ $course->id }}&buy-now=true">
<input type="hidden" id="unique-code" value="{{ $uniqueCode }}">
<input type="hidden" id="total-payment" value="{{ ($totalPay + $uniqueCode) }}">
				...
				<div class="clearfix">
					<div class="row">
						...
						<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 col-12 pl-sm-2">
							...
							<p>
								<span>@lang('label.instructor') : {{ $course->user->name }}</span> <br>
								<span>@lang('label.course_institute') : {{ $course->user->company->Name }}</span>
							</p>
						</div>
						...
						<div class="col-sm-12 col-md-3 col-lg-3 col-xl-3 col-12 pl-sm-2 mb-sm-2">
							...
							<div>
								<span>@lang('label.course_active_period') : <b>{{ $course->periode }} {{ coursePeriode($course->periode_type) }}</b> <i class="fas fa-question-circle config-tooltip cursor-area text-color" data-toggle="tooltip" data-placement="top" title="@lang('label.course_active_period_description')"></i></span>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="card card-custom">
				<div class="card-body">
					...
					<div class="clearfix">
						...
						<b>Peserta Bergabung :</b>
					</div>

					<hr>
					<div class="clearfix">
						<div class="float-right">
							<b>{{ rupiah($totalPay + $uniqueCode) }}</b>
						</div>
	...
	$(document).on('click', '#complate-payment', function () {
		...
		$.ajax({
		    ...
		    success: data => {
		        ...
		        window.location = `${baseUrl}/management/checkout/show/${data.data.id}`
		    },
		    error: e => {
		        console.log(e)

...
<input type="hidden" id="class-list-url" value="{{ route('not.logged.in.majors.index') }}">
<input type="hidden" id="reviews-list-url" value="{{ route('not.logged.in.rating.index') }}">
						...
						<div class="col-12 col-md-9 col-lg-9 col-xl-9 col-12">
							@if ($course->is_private == 0)
								<div class="float-right">
									<a href="{{ route('global.buy.course.package', 'course='.$course->slug) }}" class="btn btn-primary btn-sm w-100">
										@lang('label.buy_now')
									</a>
								</div>
							@endif
...
<script>
	function showTheory(majorId) {
		...
		$.ajax({
		    url: `${baseUrl}/not-logged-in/endpoint/subject/index?majorId=${majorId}`,
		    type: 'GET',
		    headers: {'X-CSRF-TOKEN': `${csrfToken}`},
		    success: data => {
		    	setTimeout(function () {

...
<div class="container mb-4">
	...
	<div class="card card-custom">
		<div class="card-header bg-white">
			...
			<b>Setting Autocomplate - Detail Transaksi</b>
		</div>
...
<script>
	...
	function listAutocomplate(categoryId = '', shows = '') {
		$.ajax({
		    url: `${baseUrl}/admin-panel/config-autocomplete-transaction/autocompletes?categoryId=${categoryId}`,
		    type: 'GET',
		    ...
		    success: data => {
		    	...
		    	if ((data.data.autocomplates).length > 0) {
		    		$.each(data.data.autocomplates, function(key, val) {
		    			...
		    			if ((val.root).length > 0) {
		    				const lengthRoot = (val.root).length

		    				$.each(val.root, function (key, val) {
		    					roots += ` <span>${val.value} ${(val.metric_length) ? '(' + val.metric_length + ')' : ''}</span> ${(lengthRoot > key + 1) ? ' - ' : ''}`
		    				})
		    			}
		    				...
		    				<div class="form-group">
		    					<input type="checkbox" name="autocompletes" id="autocompletes-${val.id}" value="${val.id}" ${select} ${dis}> 
		    						<span class="${(select) ? 'text-success' : ''}">
		    							${val.value} ${(val.metric_length) ? '(' + val.metric_length + ')' : ''} ${(roots) ? ' - ' + roots : '' }
		    						</span>
		    				</div>
		    			</div>`
		    		})
	...
	$(document).on('click', '#btn-save', function (e) {
		...
		$.ajax({
		    url: `${baseUrl}/admin-panel/config-autocomplete-transaction/store`,
		    type: 'POST',
		    headers: {'X-CSRF-TOKEN': `${csrfToken}`},
		    data: {
		    	categoryId,
